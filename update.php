...
if (isset($_POST['save'])) {
	...
	$tt=$e * $d;
	$sql=$ne->query("UPDATE purchase SET `date`='$a', suplyer='$b',product_name='$c', quantity='$d', price='$e', status='$f', carriage_in_ward='$car', total_price='$tt' WHERE p_id='$_GET[p_id]'")or die(mysqli_error());
	...
	$sql1=$ne->query("UPDATE products SET product_name='$c', quantity='$d',price='$e' WHERE product_name='$c'")or die(mysqli_error());
		echo "<script>alert('Process To Update Purchase Well Done!!!');window.location.replace('manage-purchase.php')</script>";


	}
	else{
		echo "<script>alert('Process To Update Purchase Was  Done Well!!!');window.location.replace('manage-purchase.php')</script>";
...
function deletePurchase(){
	$ne=new mysqli("localhost","root","","iba_d")or die(mysqli_error());
	$del=$ne->query("DELETE FROM purchase WHERE p_id='$_GET[p_id]'")or die(mysqli_error());
	if ($del == true) {
		echo "<script>alert('Success To Delete ');window.location.replace('manage-purchase.php')</script>";
	}
	else{
		echo "<script>alert('Process To  Delete was not done');window.location.replace('manage-purchase.php')</script>";
...
$ne=new mysqli("localhost","root","","iba_d")or die(mysqli_error());
	...
	$sql=$ne->query("UPDATE  returnoutward SET returnDate='$a',`date`='$date',suplyerName='$b',productName='$c',quantity='$d',price='$e',status='$f',Ticket='$tic',TotalPrice='$total' WHERE return_id='$_GET[return_id]'")or die(mysql_error());
if($sql==true){

		echo "<script>alert('Process To Update Well Done!!!');window.location.replace('manage-returnout.php')</script>";
	}
	else{

		echo "<script>alert('Process To Update Was Not Done!!!');window.location.replace('manage-	returnout.php')</script>";
